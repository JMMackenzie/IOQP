...
async fn main() -> anyhow::Result<()> {
    ...
    while step > 1 {
        ...
        let rate_limitter =
            std::sync::Arc::new(RateLimiter::direct(Quota::per_second(cur_tps.try_into()?)));
        ...
        let mut buffered_rate_limited_qrys = rate_limited_requests.buffered(cur_tps as usize);

        let mut num_errors: usize = 0;
        let mut num_processed: usize = 0;
        let mut hist = Vec::new();
        let limit = total_req_duration.as_secs() * cur_tps as u64;
        ...
        let real_tps = num_processed as f64 / start.elapsed().as_secs_f64();
        info!("real tps: {:.2}", real_tps);
        if real_tps > max_tps {
            max_concurrency = cur_tps;
            cur_tps += step;
            max_tps = real_tps;
            info!("found new max_tps: {:.2}", max_tps);
        } else {
            info!("slower than before! lets decrease concurrency!");
            step /= 2;
            cur_tps -= step;
        }
    ...
    info!(
        "MAX_TPS = {:.2} , MAX_CONCURRENCY: {}",
        max_tps, max_concurrency
    );
